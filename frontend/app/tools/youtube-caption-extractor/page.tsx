...
const initVideoId = 'GpI68hQ3acM';

export default function YoutubeCaptionExtractor() {
  const [videoId, setVideoId] = useState<string>(initVideoId);
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      const data = await window.ipc.invoke('get-subtitles', extractedVideoId);
      if (!data) {
        throw new Error(`Falsy data ${data}`);
      }
      setCaptions(data || []);
    } catch (error: any) {
  ...
  const handleCopySubtitles = () => {
    ...
    navigator.clipboard.writeText(formattedSubtitles)
      .then(() => {
        setShowToast(true);
  ...
  const sendMessageToOpenAI = async (messages: any[]) => {
    console.log('fetch openai disabled');
    return;
  };
  ...
  const handleChapterizeSubtitles = async () => {
    ...
    try {
      ...
      console.log('sendMessageToOpenAI disabled');
      return;
    } catch (error) {
      setError(`Error: ${error.message}`);
    } finally {
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            {chapters.map((chapter, index) => (
              <div key={index} style={{ marginBottom: '10px' }}>
                ...
                >
                  <strong>{chapter.startingTime} {chapter.title}</strong></a>
